  ...
  void _calculateTrend() {
    ...
    for (int i = 5; i >= 0; i--) {
      ...
      for (var doc in widget.subscriptions) {
        ...
        if (createdAt.isBefore(DateTime(now.year, now.month - i + 1, 1))) { monthTotal += cycle == 'Annuale' ? price / 12 : price; }
      }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: LineChart(
                          LineChartData(
                            minY: 0, maxY: _maxSpend,
                            gridData: FlGridData(show: true, drawVerticalLine: false, horizontalInterval: chartInterval, getDrawingHorizontalLine: (value) => const FlLine(color: Color(0xFFF0F0F0), strokeWidth: 1.5)),
                            ...
                            lineBarsData: [
                              LineChartBarData(
                                spots: _spots, isCurved: true, curveSmoothness: 0.35, color: CupertinoColors.activeBlue, barWidth: 3, isStrokeCapRound: true, dotData: const FlDotData(show: false),
                                belowBarData: BarAreaData(show: true, gradient: LinearGradient(colors: [CupertinoColors.activeBlue.withValues(alpha: 0.25), CupertinoColors.activeBlue.withValues(alpha: 0.0)], begin: Alignment.topCenter, end: Alignment.bottomCenter)),
                              ),
                            ],
                          ),
                        ),
